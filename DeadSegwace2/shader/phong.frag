...
void main(void)
{
	if(length(ex_camerapos - ex_position) > 180)
	{
	...
	{
	//shadowmapping:
	vec4 shadowtex_coords = ex_shadowmapcoords/ex_shadowmapcoords.w;
	shadowtex_coords.y=shadowtex_coords.y*(-1);
	float shadowdepth = texture(in_shadowmap,shadowtex_coords.xy*0.5+0.5).r;
	if(shadowdepth > shadowtex_coords.z)
	{
		notinshadow = false;
	}
	}
	...
	//debug: rote farbe wo ueberhaupt tiefeninformation vorliegt
	/*if(shadowdepth>0.001)
	{
	out_color = vec4(shadowdepth,0,0,1);
	}*/
}
